...
# ⛔ Harus langsung di sini — tidak boleh di bawah perintah Streamlit lain!
st.set_page_config(page_title="Prediksi Harga Mobil + Chat AI", layout="wide")

# Setelah ini baru boleh load dotenv dan kode lainnya
load_dotenv()
...
# Pilihan provider AI
ai_provider = st.sidebar.selectbox("Pilih provider AI", ["openai", "gemini"], index=1)
openai_key_env = st.sidebar.text_input("Masukkan API Key (opsional)", value="", type="password")

# Gunakan dari environment jika kosong
...
with left:
    ...
    with st.form("input_form"):
        ...
        with col1:
            model_name = st.text_input("Model", value=example_schema.get("model", "Yaris"))
            year = st.number_input("Tahun", min_value=1990, max_value=2025, value=int(example_schema.get("year", 2019)))
...
with right:
    ...
    if ask_btn and user_input.strip():
        ...
        if not openai_key_env:
            st.error("API Key tidak ditemukan. Harap masukkan di sidebar atau file .env")
        else:
            try:
